...
CREATE TABLE IF NOT EXISTS `Platto_database`.`Command_details` (
  ...
  INDEX `fk_Command_has_Menu_Command1_idx` (`command_id` ASC, `table_id` ASC, `user_id` ASC),
  CONSTRAINT `fk_Command_has_Menu_Command1`
    FOREIGN KEY (`command_id` , `table_id` , `user_id`)
    REFERENCES `Platto_database`.`Command` (`command_id` , `table_id` , `user_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
